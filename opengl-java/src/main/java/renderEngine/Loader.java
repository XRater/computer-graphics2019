...
public class Loader {
    ...
    public int loadTexture(String fileName) {
        Texture texture = new Texture("src/main/resources/texture/2d/" + fileName + ".png");
        textures.add(texture.getTextureID());
        return texture.getTextureID();
    }

    public int loadCubeMap(String folder) {
        int texID = GL11.glGenTextures();
        GL13.glActiveTexture(GL13.GL_TEXTURE0);
        GL11.glBindTexture(GL13.GL_TEXTURE_CUBE_MAP, texID);

        for (int i = 0; i < 6; i++) {
            TextureData data = decodeTextureFile("src/main/resources/texture/cube/" + folder + "/" + i + ".png");
